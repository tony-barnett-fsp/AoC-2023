private Dictionary<int, Dictionary<string, int>> _minCubes = new Dictionary<int, Dictionary<string, int>>();
...
"\nGame 100: 5 blue, 5 green; 7 blue, 15 green; 4 red, 7 green, 12 blue; 7 green, 1 blue; 5 blue, 9 green, 1 red";
	...
	foreach(var line in lines)
	{
		...
		_minCubes.Add(gameNumber, new Dictionary<string, int>
		{
			{"red", 0},
			{"blue", 0},
			{"green", 0},
		...
		{			
			foreach(var result in game.Split(new char[]{ ',' }))
			{
				...
				if(_minCubes[gameNumber][colour] < resultNumber)
				{
					_minCubes[gameNumber][colour] = resultNumber;
				}
			}
		}
	}
	
	var output = _minCubes.Select(x => new Dictionary<int, int>{ {x.Key, x.Value.Select(y => y.Value).Aggregate((y, z) => y * z)}});
	//output.Dump();
	
	
	var finalOutput = output.Select(x => x.Values.Sum()).Sum();
	finalOutput.Dump();
	//finalOutput.Dump();
}