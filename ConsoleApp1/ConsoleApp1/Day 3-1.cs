...
public class Day3
{
    ...
    private bool IsNumber(char character)
    {
        ...
        return character >= zeroInAscii && character <= nineInAscii;
    }
            ...
            for (var index = 0; index < line.Length; index++)
            {
                ...
                if (isNumber || index == line.Length - 1)
                {
                    ...
                    if (int.TryParse(currentNumber, out currentNum)
                        && (previousLine[OneCharBefore..oneCharAfter].Any(c => c != dotInAscii && IsNumber(c)
                            || nextLine[OneCharBefore..oneCharAfter].Any(c => c != dotInAscii && IsNumber(c)
                            || line[OneCharBefore..oneCharAfter].Any(c => c != dotInAscii && IsNumber(c))))))
                    {
    ...
    private Range GetRangeOfNumber(string line, int index)
    {
        var startIndex = index;
     
        while (startIndex > 0 && IsNumber(line[startIndex - 1]))
        {
            startIndex--;
        }
    ...
    private List<int> GetNumbersAdjacentToStart(int oneCharBefore, int oneCharAfter, string line)
    {
        var adjacentNumbers = new List<int>();
        var tryGetNumbers = true;
        if (IsNumber(line[oneCharBefore]))
        {
            var previousNumberIndex = GetRangeOfNumber(line, oneCharBefore);
            adjacentNumbers.Add(int.Parse(line[previousNumberIndex]));
            tryGetNumbers = previousNumberIndex.End.Value < oneCharAfter;
        }

        if (tryGetNumbers && IsNumber(line[oneCharAfter]))
        {
            var previousNumberIndex = GetRangeOfNumber(line, oneCharAfter);
            adjacentNumbers.Add(int.Parse(line[previousNumberIndex]));
        }
    ...
    public void Part2()
    {
        ...
        for (var lineIndex = 0; lineIndex < lines.Count(); lineIndex++)
        {
            ...
            for (var index = 0; index < line.Length; index++)
            {
                ...
                if (currentChar == starInnAscii)
                {
                    var oneCharBefore = index == 0 ? 0 : index - 1;
                    var oneCharAfter = index == line.Length - 1 ? line.Length - 1 : index + 1;
                    var adjacentNumbers = new List<int>();

                    adjacentNumbers.AddRange(GetNumbersAdjacentToStart(oneCharBefore, oneCharAfter, previousLine));
                    adjacentNumbers.AddRange(GetNumbersAdjacentToStart(oneCharBefore, oneCharAfter, line));
                    
                    if (adjacentNumbers.Count > 2)
                    { continue; }
                    
                    adjacentNumbers.AddRange(GetNumbersAdjacentToStart(oneCharBefore, oneCharAfter, nextLine));
                    
                    if (adjacentNumbers.Count != 2)
                    { continue; }

                    output += adjacentNumbers[0] * adjacentNumbers[1];
                }
