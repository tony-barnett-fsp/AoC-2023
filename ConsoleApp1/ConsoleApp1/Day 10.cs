...
namespace ConsoleApp1
{
    internal class Day10
    {
        ...
        private bool NextDirectionIsValid(Direction previousDirection, char currentSymbol)
        {
            var isValid = previousDirection switch
            {
                Direction.Up => "|F7".Contains(currentSymbol),
                Direction.Down => "LJ|".Contains(currentSymbol),
                Direction.Left => "L-F".Contains(currentSymbol),
                Direction.Right => "J-7".Contains(currentSymbol),
                _ => throw new Exception()
            };
            return isValid;
        }

        private Direction GetNextDirection(Direction previousDirection, char currentSymbol)
        {
            //currentCoordinates.Item1 - previousCoordinates.Item1;

            var nextDirection = currentSymbol switch
            {
                '|' => previousDirection,
                '-' => previousDirection,
                'L' => previousDirection == Direction.Down ? Direction.Right : Direction.Up,
                'J' => previousDirection == Direction.Down ? Direction.Left : Direction.Up,
                'F' => previousDirection == Direction.Up ? Direction.Right : Direction.Down,
                '7' => previousDirection == Direction.Up ? Direction.Left : Direction.Down,
                _ => throw new Exception()
            };
