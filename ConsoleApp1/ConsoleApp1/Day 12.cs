...
namespace ConsoleApp1
{
    internal class Day12
    {
        ...
        //private char[] getNextPattern(char[] perm)
        //{
        //    var n = perm.Length;
        //    var j = n - 2;
        ...
        {
            var x = Enumerable
                .Range(0, unknownCount - hashesRequired)
                .Select(na => '.')
                .ToList();

            x.AddRange(Enumerable
                    .Range(0, hashesRequired)
                    .Select(na => '#'));

            var testPattern = x.ToArray();

            return Helpers.GetPermutations(testPattern);
            //var finalTestPattern = testPattern.Reverse().ToArray();
            //var end = false;
            //while (!testPattern.All(c => c == '#'))
            //{
            //    if (testPattern.Count(c => c == '#') == hashesRequired)
            //    {
            //        yield return testPattern;
            //    }
            //    testPattern = getNextPattern(testPattern);
            //}
        }
        ...
        public void Part2()
        {
            ...
            {
                var start = Stopwatch.GetTimestamp();
                //Console.WriteLine($"starting pattern {line}");
                var currentOutput = 0;
                var x = line.Split(new char[] { ' ', }, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                ...
                foreach (var tp in testPatterns)
                {
                    //Console.WriteLine($"trying {string.Join(',', tp)}");
